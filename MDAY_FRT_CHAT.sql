...
from (
	...
	left join 
		(select ticket_id as _id_, response_time_first as frt_calendar, 'chat' as channel from zendesk_chat_insights) zen_tickets on _id_::varchar = ticket_id::varchar
	where 
		client_account = 'urbanstems'
		and (status = 'closed' or status = 'solved')
		and zendesk_chat_tickets.channel = 'chat'
